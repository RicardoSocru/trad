...
var Arreglo_formato = ["0000000", "0100000", "000", "0110011", "011", "0000011", "0100011","1100011","001","1101111"];
...
function convertirCodigo(codigo1,pos) {
    ...
    else if(codigo1.includes("addi")) {
        console.log("encontro addi");
        bin = funaddi(codigo1);
    }
...
function funaddi(codigo) {
    var i = 1;
    var num = 1;
    var binario;
    let bin = [];//arreglo de inmediate de 12 bits 
    while (i < codigo.length) {
        var parte = codigo[i];//empezamos desde la posicion 1 por que la posicion 0 ya se sabe que es addi
        if(parte!=" "){
            if (!isNaN(parte)) {
                if (num == 1) {
                    //como es el primer numero entonces es el rd 
